...
- (NSArray *)possibleNamesWithinRanking:(NSUInteger)ranking {
    ...
    [colorNames enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        if (![self getRed:&red green:&green blue:&blue alpha:&alpha]) {
            CGFloat white = 0.0;
            BOOL grayScale = [self getWhite:&white alpha:&alpha];
            if (grayScale) {
                red = white;
                green = white;
                blue = white;
        ...
        // Calculate standard deviation between reference colors and current color
        CGFloat standardDeviation = sqrtf((powf(refRed - red, 2) + powf(refGreen - green, 2) + powf(refBlue - blue, 2) + powf(refAlpha - alpha, 2)) / 4);
...
+ (void)initializeColorNames {
    NSString *plistPath = [[NSBundle mainBundle] pathForResource:@"ColorNames" ofType:@"plist"];
